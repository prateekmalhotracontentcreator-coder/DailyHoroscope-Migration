...
> Last updated: 2026-06-09 (Growth GRW-1→SOCIAL-1 CC-verified; 8 TT Render env vars added as M-39 through M-46. KE ALL LK OPS CLOSED. KE-OP-LK-1: dedup clean. KE-OP-LK-2: 17 rules closed (13 AA + 4 rejected). LK batch: 304 AA / 0 flagged / 4 rejected / 467 total. Cross-system engine architecture note locked. DO NOT re-review SBC, BPHS Ch48, LK dedup, or LK Bucket C.)
...
| M-6 | **Instagram Business Account ID** -- not loading in Meta dashboard. Resolve to enable Instagram posting from Admin Console. See also M-41 (SOCIAL-1 commission now built with guard state -- just needs env var) | Instagram posting blocked | 🟡 MED |
...
| ~~M-35~~ | ~~**KE Phase 1 Ingest -- 300 Combinations**~~ -- ✅ **DONE 2026-06-01.** 329 rules ingested (batch `300-combinations-v1-20260601`). Triage complete: 141 AA / 188 PHR / 0 flagged. Retroactive dedup clean. | ✅ DONE | ✅ |
...
| M-39 | **GRW-3: Add `SERPER_API_KEY` to Render env vars** -- unlocks SERPER keyword intel weekly job + SERPER sub-tab in Intelligence Dashboard | GRW-3 SERPER live validation blocked | 🟠 HIGH |
| M-40 | **GRW-3: Add `GSC_CLIENT_ID` + `GSC_CLIENT_SECRET` to Render env vars** -- then complete GSC OAuth in Admin → Intelligence → GSC tab | GRW-3 GSC Index Health blocked | 🟠 HIGH |
| M-41 | **SOCIAL-1: Confirm `INSTAGRAM_BUSINESS_ACCOUNT_ID`** in Meta Business Manager → add to Render env vars | Instagram posting blocked (SOCIAL-1 G-01 gate) | 🟡 MED |
| M-42 | **SOCIAL-1: Create Twitter Developer App** → generate `TWITTER_API_KEY`, `TWITTER_API_SECRET`, `TWITTER_ACCESS_TOKEN`, `TWITTER_ACCESS_TOKEN_SECRET` → add all 4 to Render env vars | X posting blocked (SOCIAL-1 G-04 gate) | 🟡 MED |
| M-43 | **GRW-1: Verify `transit_alerts_consent` opt-in checkbox** appears on Birth Profile form in production. Then trigger `GET /api/admin/transit-segments/summary` in Admin → Notifications → Transit Campaigns tab | GRW-1 live validation | 🟡 MED |
| M-44 | **GRW-2: Trigger a test payment on Render** and confirm Day 0 lifecycle email arrives within 60 seconds. Check `lifecycle_sequences` collection in MongoDB | GRW-2 live validation | 🟡 MED |
| M-45 | **GRW-4: Open Leads tab on Render**, add a test lead (company name = "Test Co"), change stage via badge dropdown, verify stage updates without reload | GRW-4 live validation | 🟡 MED |
| M-46 | **Ads / Header Bidding** -- deferred to Phase 2. Needs ~10k monthly sessions threshold + co-founder approval before issuing | Phase 2 gate | 🔵 PHASE 2 |
