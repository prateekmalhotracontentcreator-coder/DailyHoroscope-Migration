...
| M-16 | **LSG-1 integration** -- Lo Shu Grid delivered locally but **⚠️ DO NOT integrate yet**. Pre-integration scan (`python3 tests/echo_pace_lsg_scan.py`) shows combo pages BLOCKED (45 pairs ≥70% cosine -- content is just concatenated number bodies). Operations thread must fix combo page prose first → re-run scanner → confirm PASS → then TT integrates | Combo page content fix required before integration | 🟠 HIGH |
