...
| M-36 | **TT decisions required before ingest** -- (a) 300 Horoscopes: 3 blocked rules (h300-s01-016, h300-s04-004, h300-s04-005) -- approve/reject/modify each. (b) SBC: 7 blocking priority conflicts from 40-question batch -- review and resolve. (c) Longevity Unnatural: lu-s04-001 ("must" vs "should") + lu-s04-014 (AND vs OR) -- architecture decisions. ~~(d) BPHS Vol 1 doctrinal ambiguities~~ -- ✅ ALL 10 HIGH items resolved by GAI 2026-05-30. Resolutions + Python engine code validated at `BPHS_CC_Decode/BPHS_Vol1_GAI_Resolutions.md` + `BPHS_Vol1_Engine_Core.py`. | TT to resolve (a)(b)(c) only | 🟠 HIGH |
| M-37 | **Aayu bucket methodology co-founder sign-off** -- Longevity 58-chapter ingest (~600+ rules) requires explicit approval on the aayu bucket methodology (longevity span calculation). Do NOT allow ingest to begin without this sign-off. | TT (co-founder) | 🟠 HIGH |
