...
## ECHO // PACE Compliance KPIs -- Mandatory Before Delivery (Addendum 2026-05-27)

Every page batch delivered by Codex must pass all 3 layers of the E.C.H.O. // P.A.C.E. machine before integration. Run the compliance verifier and paste results in your delivery confirmation.

### 3-Layer Architecture

| Layer | Method | Catches | Pass Threshold |
|---|---|---|---|
| Layer 1 | TF-IDF Cosine (body fields only) | Overall structural similarity -- paraphrasing, template copying | < 40% (FLAGGED ≥50%, BLOCKED ≥70%) |
| Layer 2 | N-gram phrase match (stop-word filtered) | 4+ consecutive meaningful words verbatim across multiple records | No 4-gram phrase in > 15% of records |
| Layer 3 | Jaccard heading match | Page titles, section headings, scripture labels that are verbatim from source text or repeated across pages | Jaccard < 75% across any pair |

### Faith-Specific Stop Words for Layer 1

```python
FAITH_STOP_WORDS = [
    "chapter", "verse", "shloka", "gita", "bhagavad", "bible", "scripture",
    "testament", "lord", "god", "krishna", "arjuna", "christ", "jesus",
    "holy", "faith", "unto", "shall", "thee", "thou", "recitation", "meaning",
    "translation", "prayer", "spiritual", "teaching", "wisdom", "divine",
    "says", "said", "speaks", "written", "reads", "passage", "text",
]
```

### Layer 2 Rule for Faith

Sacred text phrases (e.g. direct Gita shloka translations, Bible verse text) are
the highest Layer 2 risk. Each page must open with 80-100 words in situation
vocabulary BEFORE quoting scripture -- this is the Linguistic Isolation Rule from
PROCESS_5. Pages that open with scripture verbatim will fail Layer 2.

### Layer 3 Rule for Faith

Section headings such as "What Does This Verse Mean", "How This Applies Today",
"A Modern Reading" must not be verbatim across all 10,500 Gita pages or all
6,000 Bible pages. At least 5 distinct heading variants per section type required.
...
(Temple Team will create `verify_faith_compliance.py` after Phase 1A delivery.)

### Acceptance Gate

All 3 layers must show PASS or OVER TARGET (never FLAGGED or BLOCKED) before
Temple Team will integrate the delivery. Include test output in your delivery message.
