...
| **ANGEL-2** | Angel Numbers Generator Rewrite -- fix all 3 ECHO/PACE failure modes + `how_to_manifest` field for manifestation records | Angel Numbers (same thread) | `Angel_Numbers/CODEX_COMMISSION_ANGEL_2_REWRITE.md` | 🔴 READY TO ISSUE | ANGEL-1 ✅ | **Issue now.** Blocks seed + deploy. Brief fully written. After delivery: run compliance check, then re-seed both collections on Render. |
