...
| STR-OP-22 | TT decision: remove or retain proof strips on live Action Plan page | TT | 🟡 LOW | `OracleVerdictProofStrip` + `LKStatusChipProofStrip` render at bottom of §03 Verdict and §02 Diagnostics. CD canvas components. TT to decide if they should render for live users. |
| STR-OP-23 | Verify `gate_summaries` API field shape vs LKGateSummaries expectations | CC | 🟠 HIGH | LKGateSummaries expects gates `{ id, code, name, facet, status, narrative, asideLabel, asideValue }`. If backend doesn't return this shape, §02 Diagnostics renders blank. Verify against live `/api/strategist/dashboard` response. |
