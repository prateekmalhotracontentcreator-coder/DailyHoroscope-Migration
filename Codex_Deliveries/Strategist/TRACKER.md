...
| Field | Value |
|---|---|
| **Status** | 🟢 WRS H-SCROLL LIVE -- `WarRoomHScrollPage.jsx` at `/strategist/war-room`: full-viewport horizontal snap-scroll, 5 panels, 4 theme modes. STR-2H: 7 notification triggers wired (`strategist_notification_service.py` + `notification_trigger_router.py`). All Phase 2 + Doc 10 items closed. Pending: TT production verify (STR-OP-24/25) and STR-2H trigger smoke test (STR-OP-29). |
...
| **STR-CD-WRS** | War Room Horizontal Snap-Scroll (5-panel full-viewport layout, sticky Gate 0, 4 theme modes) | ✅ INTEGRATED -- commit `71eb351` | `CODEX_COMMISSION_STR_CD_WAR_ROOM_H_SCROLL.md` · Built directly by CC 2026-06-03. `WarRoomHScrollPage.jsx` + `war-room-hscroll.css`. Route `/strategist/war-room` now serves H-scroll page. |
| **STR-2H** | 7 Notification Triggers (golden_hour_open/egress, hurdle_raised, ritual_reminder, verdict_change, mission_complete, dasha_transition) | ✅ INTEGRATED -- commit `71eb351` | Doc 11 · `strategist_notification_service.py` (evaluators + dedup) + `notification_trigger_router.py` (7 TRIGGER_CONFIG entries). |
...
| ~~STR-OP-20~~ | ~~CD commission: War Room horizontal scroll layout (STR-CD-WRS)~~ | -- | ✅ DONE 2026-06-03 | Brief was `CODEX_COMMISSION_STR_CD_WAR_ROOM_H_SCROLL.md`. CC built directly: `WarRoomHScrollPage.jsx` + `war-room-hscroll.css`. Route live. Commit `71eb351`. |
...
| ~~STR-OP-28~~ | ~~Doc 10 §03 wiring: ConquestScoreboard + LKGateSummaries mounted on snapshot page~~ | CC | ✅ DONE 2026-06-03 | `StrategistWarRoomPage.jsx` now imports 2F/2E components + CSS, adds `normalizeGates()` adapter, adds `scoreboardData`/`gateSummaries` to `mapWarRoomProps()`, renders both below `<StrategistWarRoom/>`. Commit `ff57997`. Build clean. Last acceptance checklist item from Doc 10 now closed. |
...
| v2.8 | 2026-06-03 | STR-CD-WRS H-scroll War Room integrated. `WarRoomHScrollPage.jsx` (new, ~750 lines): full-viewport horizontal snap-scroll, 5 panels (Conquest/Missions/Diagnostics/Scoreboard/Timeline), sticky Gate 0 header, dot/arrow nav with 440ms guaranteed fallback, keyboard ← → nav, localStorage panel restore, 4 theme modes. `war-room-hscroll.css` (new): `.wrs-*` prefixed CSS, all theme token blocks, all panel layouts. App.js: `/strategist/war-room` lazy-imports WarRoomHScrollPage (was StrategistPage). STR-2H: `strategist_notification_service.py` (new): 7 async evaluators with condition checks + dedup via `str_notification_log`. `notification_trigger_router.py`: 7 new `str-*` TRIGGER_CONFIG entries with locked push copy per Doc 11. STR-OP-20 + STR-OP-26 closed. STR-OP-29 added. ENGINE_VERSION bumped to v21-str2h-wrs. Commit `71eb351`. | CC | -- |
| v2.7 | 2026-06-03 | Doc 10 §03 wiring applied. `StrategistWarRoomPage.jsx` (snapshot page) now mounts `ConquestScoreboard` (2F) + `LKGateSummaries` (2E) below `<StrategistWarRoom/>`. `normalizeGates()` adapter added (same logic as ActionPlanPage). CSS imports for 2F + 2E added. All acceptance checklist items from Integration Manifest (Doc 10) now closed. | CC | `ff57997` |
| v2.6 | 2026-06-02 | STR-UM-01 Field Manual integrated. `StrategistManualPage.jsx` (React component, 10 sections §00-§09) + `strategist-manual.css` (React-adapted from CD, body[data-mode] → .str-manual[data-mode], sticky rail/mbar corrected for NavBar height, --gold token injected). 4-theme toggle (light/dark/cr-ambient/cr-tactical) with localStorage persistence. IntersectionObserver scroll-spy wires rail active-state + mobile dropdown. Route `/strategist/manual` added (ProtectedRoute, lazy). "Field Manual" link added to snapshot page nav strip + StrategistPage layer-pill bar. STR-OP-27 closed. Build clean. | CC | `a8bd50e` |
