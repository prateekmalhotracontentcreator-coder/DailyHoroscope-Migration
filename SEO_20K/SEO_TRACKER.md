...
> Last updated: 2026-06-02 · v6.0 (M3-CP-FIX v7 issued. v6 scan: L1 96.1% BLOCKED. "style outer style rising" 22% from "outer style" 2-word lens + Pattern B cross-house clustering L3. v7: _CP_LENS_WORD single-word dict, single title template with house + sign-adj, delete Pattern B.)
...
| v6.0 | 2026-06-02 | M3-CP-FIX v6 re-scan: L1 96.1% BLOCKED. "style outer style rising" 22% -- chart_point[lens].split(,)[0] for rising = "outer style" (2 words) creates double-style 4-gram in summary. L3: Pattern B has no house in title, so same sign+chartpoint across 12 houses shares 5/6 tokens = Jaccard 71%. v7 brief: add _CP_LENS_WORD = {sun:identity, moon:emotion, rising:presence}, update summary f-string (replaces lens fragment), change _CP_TITLE_LABEL[rising] to "Ascendant Presence", delete Pattern B, use single title template with all 4 axes (sign+cp+house+adj+topic). Jaccard verified 55% on all 3 axes. | CC | `CODEX_COMMISSION_M3_CP_FIX.md` v7 |
| v5.9 | 2026-06-02 | M3-CP-FIX v5-FULL re-scan: L1 92.4% BLOCKED. FAQ fix confirmed (both hardcoded strings gone). Summary _SUMMARY_TEMPLATES lambdas failing at 35%: fixed closing clauses appear on 144/432 pages per template. L3: Pattern B full house topic string causes Jaccard ~75%. v6 brief: delete _SUMMARY_TEMPLATES, use single dense f-string (worst 4-gram 8.3%); add _CP_TITLE_LABEL 2-word labels (Solar Identity/Lunar Instinct/Ascendant Style) + truncate house topic to first word only -- Jaccard ~50% both axes. | CC | `CODEX_COMMISSION_M3_CP_FIX.md` v6 |
| v5.8 | 2026-06-02 | M3-CP-FIX v5-FULL brief issued. Baseline mismatch confirmed: v3-v4 were uncommitted Codex thread edits, never in repo. Actual repo baseline = 93.4% original. Read _placement_traits() directly -- root cause identified: 2 hardcoded FAQ answer strings ("strengths and shadows that become clearer" 100%, "rising house placements especially" 100%) plus fixed summary stems. v5-FULL brief targets original file: Fix A (2 FAQ answer dicts, per-chartpoint 3-variant pools), Fix B (summary 3-variant lambda list), Fix C (chart-point vocab in existing f-strings), Fix D (sign-adjective title). Hard constraint: no new functions beyond _hash_index and 4 data constants. | CC | `CODEX_COMMISSION_M3_CP_FIX.md` v5-FULL |
| v5.7 | 2026-06-02 | M3-CP-FIX v4 re-scan: L1 96.1% BLOCKED (regression from 52.0%). Root cause: new sign-family helper functions introduced universal stems ("house story tends feel" 100%, "behaviour often sharpens native" 100%, "house reading generically gives" 100%). Working tree reverted to v3 state (52.0%). v5 brief issued -- data-only edits, no new code structure, 3 surgical phrase deletions + sign-adjective title fix. | CC | `CODEX_COMMISSION_M3_CP_FIX.md` v5 |
