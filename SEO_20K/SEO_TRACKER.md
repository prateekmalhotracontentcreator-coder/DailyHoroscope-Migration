...
> Last updated: 2026-06-02 · v5.7 (M3-CP-FIX v4 regressed to 96.1% BLOCKED -- new helper functions introduced universal stems. Working tree reverted to v3 (52.0%). v5 brief issued: data-only edits to v3, no new code, 3 surgical phrase deletions + one-line title adjective fix.)
...
| v5.7 | 2026-06-02 | M3-CP-FIX v4 re-scan: L1 96.1% BLOCKED (regression from 52.0%). Root cause: new sign-family helper functions introduced universal stems ("house story tends feel" 100%, "behaviour often sharpens native" 100%, "house reading generically gives" 100%). Working tree reverted to v3 state (52.0%). v5 brief issued: data-only edits, no new code structure, 3 surgical phrase deletions from existing pools + one-line sign-adjective title fix via module-level dict. Key insight: every new helper function collapses to BLOCKED; v3 succeeded with direct data edits. | CC | `CODEX_COMMISSION_M3_CP_FIX.md` v5 |
