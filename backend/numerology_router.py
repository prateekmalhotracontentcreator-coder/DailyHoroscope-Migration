...
def _build_timing_report(payload: NumerologyGenerateRequest, user_email: str) -> NumerologyReportPayload:
    ...
    return NumerologyReportPayload(**{**_report_base(payload, user_email, "numerology_annual_forecast", "favorable-timing-major-decisions"), "target_year": target_year}, computed_values={"personal_year": py, "life_path": lp}, calculation_trace={"personal_year": py_trace, "life_path": lp_trace, "timing_forecast": _timing_forecast_lines(target_year, py.reduced), "monthly_highlights": _monthly_highlights(py.reduced)}, report_sections=sections, summary=f"Your timing report centers on Personal Year {py.reduced} for {target_year}, read in the context of Life Path {lp.reduced}.", guidance="Use this year as a timing framework for when to advance, refine, or pause.", remedy_note="If the year feels resistant, the next layers to examine are name alignment, Lo Shu balance, or compatibility with your current environment.", lifecycle_hint=f"Revisit your timing profile during {target_year} to track shifts in momentum and planning windows.")
...
def _build_karmic_loshu_report(payload: NumerologyGenerateRequest, user_email: str) -> NumerologyReportPayload:
    ...
    sections = [NumerologyReportSection(section_id="karmic_debt_indicators", title="Karmic Debt Indicators", summary=f"Detected karmic debt indicators: {', '.join(str(k) for k in karmic) if karmic else 'none'}.", body="Karmic debt numbers are treated as recurring lesson patterns rather than punishments."), NumerologyReportSection(section_id="loshu_pattern_analysis", title="Lo Shu Pattern Analysis", summary=f"Missing numbers: {', '.join(str(n) for n in loshu_payload['missing_numbers']) if loshu_payload['missing_numbers'] else 'none'}. Repeated: {', '.join(str(n) for n in loshu_payload['repeated_numbers']) if loshu_payload['repeated_numbers'] else 'none'}.", body="Lo Shu patterns highlight where balance, repetition, or developmental emphasis may be more visible."), NumerologyReportSection(section_id="remedial_guidance", title="Remedial Guidance", summary="Use remedial steps to create steadier balance in rhythm, awareness, and self-correction.", body="Treat remedial suggestions as supportive practices rather than guarantees.")]
    return NumerologyReportPayload(**_report_base(payload, user_email, "numerology_karmic_loshu", "karmic-debt-loshu-grid-remediation"), computed_values={"life_path": lp, "expression": core["expression"], "soul_urge": core["soul_urge"], "personality": core["personality"]}, calculation_trace={"life_path": lp_trace, "karmic_debts": karmic, "lo_shu_grid_payload": loshu_payload}, report_sections=sections, summary="This report highlights karmic lesson patterns alongside Lo Shu gaps and repetitions that may point to areas needing more conscious balance.", guidance="Use remedial steps to create steadier balance in environment, rhythm, and self-correction.", remedy_note="Treat remedial suggestions as supportive practices and not as guarantees around health, money, or relationships.")
