...
def generate_birth_chart_pdf(profile: dict, report_content: str,
    ...
    if chart_data and chart_data.get('houses'):
        ...
        HOUSE_NAMES = {
            1:'Lagna — Self & Personality', 2:'Dhana — Wealth & Family',
            3:'Sahaja — Siblings & Courage', 4:'Sukha — Home & Mother',
            5:'Putra — Children & Intellect', 6:'Ripu — Health & Enemies',
            7:'Kalatra — Marriage & Partnerships', 8:'Mrityu — Longevity & Transformation',
            9:'Dharma — Fortune & Spirituality', 10:'Karma — Career & Status',
            11:'Labha — Gains & Friends', 12:'Vyaya — Liberation & Foreign Lands',
        }
        ...
        for h_num, h_data in chart_data['houses'].items():
            ...
            house_name_short = HOUSE_NAMES.get(h_num, h_data.get('name', f'House {h_num}')).split('—')[0].strip()
            house_rows.append([
...
def generate_brihat_kundli_pdf(report: dict, chart_data: dict = None,
                                password: str = None) -> BytesIO:
    """
    Generate premium Brihat Kundli Pro PDF — full 40-page structured report.
    Uniform typography, section hierarchy, prose paragraphs.
    """
    buffer = BytesIO()
    doc = SimpleDocTemplate(
        buffer, pagesize=A4,
    ...
    def prose(text, spaceBefore=2, spaceAfter=4):
        """Standard body paragraph."""
        if not text: return Spacer(1, 1*mm)
        s = ParagraphStyle('prose_b', parent=styles['body'],
                           spaceAfter=spaceAfter*mm, spaceBefore=spaceBefore*mm,
                           leading=14, fontSize=9)
        return Paragraph(str(text), s)

    def sub_heading(text):
        s = ParagraphStyle('sub_b', parent=styles['small'],
                           fontSize=8, textColor=MUTED, spaceBefore=3*mm,
                           spaceAfter=1*mm, fontName='Helvetica-Bold',
                           letterSpacing=0.8)
        return Paragraph(text.upper(), s)

    def gold_heading(text):
        s = ParagraphStyle('gold_h', parent=styles['small'],
                           fontSize=10, textColor=GOLD, spaceBefore=4*mm,
                           spaceAfter=2*mm, fontName='Helvetica-Bold')
        return Paragraph(text, s)

    def bullet_item(text):
        s = ParagraphStyle('bul_b', parent=styles['body'],
                           fontSize=9, leftIndent=10, spaceAfter=2*mm,
                           bulletIndent=2, leading=13)
        return Paragraph(f'• {text}', s)

    def kv_table(rows):
        """Simple 2-column key-value info table."""
        t_rows = []
        for k, v in rows:
            t_rows.append([
                Paragraph(str(k), ParagraphStyle('k', parent=styles['small'],
                          fontSize=8, textColor=MUTED, fontName='Helvetica-Bold')),
                Paragraph(str(v), ParagraphStyle('v', parent=styles['body'],
                          fontSize=9)),
            ])
        t = Table(t_rows, colWidths=[38*mm, CONTENT_W - 38*mm])
        t.setStyle(TableStyle([
            ('GRID',    (0,0), (-1,-1), 0.3, BORDER_GREY),
            ('ROWBACKGROUNDS', (0,0), (-1,-1), [CREAM, CREAM_DARK]),
            ('PADDING', (0,0), (-1,-1), 5),
            ('VALIGN',  (0,0), (-1,-1), 'TOP'),
        ]))
        return t

    def rating_line(label, value):
        if not value: return Spacer(1,1*mm)
        s = ParagraphStyle('rl', parent=styles['small'],
                           fontSize=9, textColor=GOLD_DARK, spaceAfter=2*mm)
        return Paragraph(f'<b>{label}:</b>  {value}', s)

    def two_col(left_items, right_items, left_title='', right_title=''):
        """Two-column bullet list."""
        def col_content(title, items):
            c = []
            if title: c.append(sub_heading(title))
            for item in (items or []):
                c.append(bullet_item(str(item)))
            return c
        t = Table([[col_content(left_title, left_items),
                    col_content(right_title, right_items)]],
    ...
    # ── Cover ─────────────────────────────────────────────────────────────────
    build_cover('BRIHAT KUNDLI PRO', name, report_date, styles,
                story, subtitle='Comprehensive Vedic Astrology Life Report')
    story.append(Spacer(1, 3*mm))
    ...
    details = [
        ...
        ['Moon Sign (Rashi)', f"{moon.get('sign','')} — {moon.get('nakshatra','')} Nakshatra Pada {moon.get('nakshatra_pada','')}"],
        ['Current Dasha',  f"{cur_d.get('mahadasha','')} Mahadasha ({cur_d.get('period','')})"],
    ]
    story.append(kv_table(details))
    story.append(Spacer(1, 4*mm))

    # ── North Indian Chart ────────────────────────────────────────────────────
    if chart_data and chart_data.get('houses'):
        story.append(section_divider('Janma Kundali — North Indian Chart', styles))
        story.append(Paragraph(
            f"Ascendant: {asc.get('sign','')} ({asc.get('sign_vedic', asc.get('sign',''))}) · "
            f"Moon: {moon.get('sign','')} · Nakshatra: {moon.get('nakshatra','')}",
            styles['small']
        ))
        story.append(Spacer(1, 3*mm))
        chart_drawing = draw_north_indian_chart(chart_data['houses'], asc.get('sign',''), size=168)
        ct = Table([[chart_drawing]], colWidths=[CONTENT_W])
    ...
    story.append(section_divider('Ascendant & Personality', styles))
    if asc.get('overview'):
        story.append(prose(asc['overview']))
    if asc.get('key_traits'):
        story.append(two_col(asc.get('key_traits',[]), asc.get('strengths',[]),
                             'Key Traits', 'Strengths'))
    if asc.get('challenges'):
        story.append(sub_heading('Challenges & Growth Areas'))
        for c in asc['challenges']:
            story.append(bullet_item(str(c)))
    story.append(Spacer(1, 4*mm))

    # ── Moon Sign & Emotional Nature ──────────────────────────────────────────
    story.append(section_divider('Moon Sign & Emotional Nature', styles))
    if moon.get('overview'):
        story.append(prose(moon['overview']))
    if moon.get('emotional_nature'):
        story.append(two_col(moon.get('emotional_nature',[]),
                             moon.get('mental_tendencies',[]),
                             'Emotional Nature', 'Mental Tendencies'))
    story.append(Spacer(1, 4*mm))

    # ── Sun Sign ──────────────────────────────────────────────────────────────
    sun = report.get('sun_sign', {})
    if sun:
        story.append(section_divider('Sun Sign & Core Identity', styles))
        if sun.get('overview'):
            story.append(prose(sun['overview']))
        story.append(Spacer(1, 3*mm))
    ...
    if planets:
        ...
        for p in planets:
            ...
            sc = '2E7D32' if 'Strong' in strength else ('E65100' if 'Good' in strength or 'Moderate' in strength else 'C62828')
            effects = p.get('effects', [])
            eff_text = effects[0] if effects else ''
            rows.append([
                Paragraph(f"<b>{p.get('planet','')}</b>", styles['small']),
                Paragraph(str(p.get('sign','')), styles['small']),
                Paragraph(f"H{p.get('house','')}", styles['small']),
                Paragraph(str(p.get('status','')), styles['small']),
                Paragraph(f'<font color="#{sc}"><b>{strength}</b></font>', styles['small']),
                Paragraph(eff_text[:90], styles['small']),
            ])
        cw = [28*mm, 22*mm, 10*mm, 28*mm, 20*mm, CONTENT_W-108*mm]
        ...
        # Additional effects
        story.append(Spacer(1, 3*mm))
        story.append(sub_heading('Detailed Planetary Effects'))
        for p in planets:
            effects = p.get('effects', [])
            if len(effects) > 1:
                story.append(gold_heading(f"{p.get('planet','')} in House {p.get('house','')} ({p.get('sign','')})"))
                for eff in effects:
                    story.append(bullet_item(str(eff)))
        story.append(Spacer(1, 4*mm))

    story.append(PageBreak())

    # ── 12-House Analysis ─────────────────────────────────────────────────────
    house_analysis = report.get('house_analysis', [])
    if house_analysis:
        story.append(section_divider('12-House Analysis', styles))
        story.append(prose(
            f"The following analysis examines all 12 houses of {fname}'s birth chart, "
            "revealing the specific areas of life governed by each house and the "
            "planetary influences at play."
        ))
        story.append(Spacer(1, 3*mm))
        for h in house_analysis:
            h_num  = h.get('house', '')
            h_name = h.get('name', f'House {h_num}')
            sign   = h.get('sign', '')
            lord   = h.get('lord', '')
            h_planets = h.get('planets', [])
            interp = h.get('interpretation', '')

            # House header
            header_text = f"House {h_num} — {h_name}"
            story.append(gold_heading(header_text))
            meta_parts = []
            if sign:   meta_parts.append(f"Sign: {sign}")
            if lord:   meta_parts.append(f"Lord: {lord}")
            if h_planets: meta_parts.append(f"Planets: {', '.join(str(p) for p in h_planets)}")
            if meta_parts:
                story.append(Paragraph(
                    ' · '.join(meta_parts),
                    ParagraphStyle('hmeta', parent=styles['small'],
                                   fontSize=8, textColor=MUTED, spaceAfter=2*mm)
                ))
            if interp:
                story.append(prose(interp))
            story.append(Spacer(1, 2*mm))

    story.append(PageBreak())

    # ── Yogas & Planetary Combinations ───────────────────────────────────────
    yogas = report.get('benefic_yogas', []) + report.get('malefic_yogas', [])
    if yogas:
        story.append(section_divider('Yogas & Planetary Combinations', styles))
        for y in yogas:
            if isinstance(y, dict):
                story.append(gold_heading(y.get('name', 'Yoga')))
                if y.get('planets_involved'):
                    story.append(Paragraph(
                        f"Planets: {', '.join(y.get('planets_involved',[]))}",
                        ParagraphStyle('yp', parent=styles['small'],
                                       fontSize=8, textColor=MUTED, spaceAfter=1*mm)
                    ))
                if y.get('effect'):
                    story.append(prose(y['effect']))
        story.append(Spacer(1, 4*mm))
    ...
    if career:
        ...
        if career.get('overall_rating') or career.get('business_potential'):
            story.append(rating_line(
                'Overall Rating',
                f"{career.get('overall_rating','')} · Business Potential: {career.get('business_potential','')}"
            ))
        if career.get('overview'):
            story.append(prose(career['overview']))
        story.append(two_col(
            career.get('best_career_fields', []),
            career.get('strengths_at_work', []),
            'Best Career Fields', 'Strengths at Work'
        ))
        timeline = career.get('career_timeline', [])
        if timeline:
            story.append(Spacer(1, 3*mm))
            story.append(sub_heading('Career Timeline'))
            for t in timeline:
                trow = Table([[
                    Paragraph(f'<b>{t.get("period","")}</b>', styles['small']),
                    [prose(t.get('prediction','')),
                     Paragraph(f'<i>💡 {t.get("advice","")}</i>',
                               ParagraphStyle('adv', parent=styles['small'],
                                              fontSize=8, textColor=MUTED, spaceAfter=2*mm))]
    ...
    if love:
        ...
        if love.get('overall_rating'):
            story.append(rating_line('Overall Rating', love.get('overall_rating','')))
        if love.get('overview'):
            story.append(prose(love['overview']))
        story.append(two_col(
            love.get('ideal_partner_traits', []),
            love.get('compatibility_signs', []) + (['Challenging: ' + ', '.join(love.get('challenging_signs',[]))] if love.get('challenging_signs') else []),
            'Ideal Partner Traits', 'Compatibility'
        ))
        mt = love.get('marriage_timing', {})
        if mt:
            years = ', '.join(str(y) for y in mt.get('favorable_years', []))
            if years:
                story.append(Paragraph(
                    f'<font color="#C5A059"><b>Favourable Marriage Years:</b></font> {years}',
                    ParagraphStyle('mt', parent=styles['small'],
                                   fontSize=9, spaceAfter=2*mm, spaceBefore=2*mm)
                ))
            if mt.get('marriage_analysis'):
                story.append(prose(mt['marriage_analysis']))
        story.append(Spacer(1, 4*mm))
    ...
    if health:
        ...
        hdr = [health.get('overall_vitality',''), health.get('body_constitution','')]
        story.append(rating_line('Constitution', ' · '.join(h for h in hdr if h)))
        if health.get('overview'):
            story.append(prose(health['overview']))
        story.append(two_col(
            health.get('vulnerable_areas', []),
            health.get('preventive_measures', []),
            'Vulnerable Areas', 'Preventive Measures'
        ))
        if health.get('dietary_recommendations'):
            story.append(sub_heading('Dietary Recommendations'))
            for d in health['dietary_recommendations']:
                story.append(bullet_item(str(d)))
        if health.get('health_timeline'):
            story.append(prose(health['health_timeline']))
        story.append(Spacer(1, 4*mm))
    ...
    if wealth:
        ...
        if wealth.get('overall_rating'):
            story.append(rating_line('Overall Rating', wealth.get('overall_rating','')))
        if wealth.get('overview'):
            story.append(prose(wealth['overview']))
        story.append(two_col(
            wealth.get('primary_income_sources', []),
            wealth.get('good_investments', []),
            'Primary Income Sources', 'Good Investments'
        ))
        avoid = wealth.get('avoid', [])
        if avoid:
            story.append(sub_heading('Avoid'))
            for a in avoid:
                story.append(bullet_item(str(a)))
        story.append(Spacer(1, 4*mm))

    # ── Family & Relationships ────────────────────────────────────────────────
    family = report.get('family_prediction', {})
    if family and family.get('overview'):
        story.append(section_divider('Family & Home Life', styles))
        story.append(prose(family.get('overview','')))
        if family.get('parents'):
            story.append(gold_heading('Parents & Parental Relationships'))
            story.append(prose(family['parents']))
        if family.get('siblings'):
            story.append(gold_heading('Siblings'))
            story.append(prose(family['siblings']))
        if family.get('children'):
            story.append(gold_heading('Children & Progeny'))
            story.append(prose(family['children']))
        story.append(Spacer(1, 4*mm))
    ...
    if cur_dasha or dasha_tl:
        story.append(section_divider('Dasha Period Analysis', styles))
        if cur_dasha:
            planet = cur_dasha.get('mahadasha', '')
            period = cur_dasha.get('period', '')
            story.append(gold_heading(f'Current: {planet} Mahadasha ({period})'))
            if cur_dasha.get('overview'):
                story.append(prose(cur_dasha['overview']))
            for eff in cur_dasha.get('effects', []):
                story.append(bullet_item(str(eff)))
            story.append(Spacer(1, 3*mm))

        if dasha_tl:
            story.append(sub_heading('Upcoming Mahadashas'))
            for d in dasha_tl:
                p_name = (d.get('planet','') or '').replace(' Mahadasha','').replace(' Dasha','')
                period = d.get('period', '–')
                story.append(gold_heading(f'{p_name} Mahadasha — {period}'))
                if d.get('overview'):
                    story.append(prose(d['overview']))
    ...
    kalsarp = report.get('kalsarp_dosha', {})
    if mangal or kalsarp:
        story.append(section_divider('Dosha Analysis', styles))
        if mangal:
            has = mangal.get('has_dosha') or mangal.get('present', False)
            severity = mangal.get('severity', '')
            color = '#C62828' if has else '#2E7D32'
            status = f'⚠ PRESENT — Severity: {severity}' if has else '✓ Not Present'
            story.append(Paragraph(
                f'<font color="{color}"><b>Mangal Dosha: {status}</b></font>',
                ParagraphStyle('md', parent=styles['body'],
                               fontSize=10, spaceAfter=2*mm, spaceBefore=2*mm)
            ))
            mars_h = mangal.get('mars_house','')
            if mars_h:
                story.append(Paragraph(f'Mars placed in House {mars_h}', styles['small']))
            if mangal.get('effects'):
                story.append(prose(mangal['effects']))
            for rem in mangal.get('remedies', []):
                story.append(bullet_item(str(rem)))
            story.append(Spacer(1, 3*mm))
        if kalsarp and kalsarp.get('has_dosha'):
            story.append(Paragraph('<font color="#C62828"><b>Kalsarp Dosha: Present</b></font>',
                                   styles['body']))
            for rem in kalsarp.get('remedies', []):
                story.append(bullet_item(str(rem)))
        story.append(Spacer(1, 4*mm))
    ...
    if gem_r or man_r or life_r:
        ...
        if gem_r:
            story.append(sub_heading('Gemstone Remedies'))
            for g in gem_r:
                if isinstance(g, dict):
                    story.append(gold_heading(f"{g.get('stone', g)} — {g.get('planet', '')}"))
                    if g.get('benefit'):
                        story.append(prose(g['benefit']))
                    if g.get('how_to_wear'):
                        story.append(Paragraph(
                            f'<i>How to wear: {g["how_to_wear"]}</i>',
                            ParagraphStyle('hw', parent=styles['small'],
                                           fontSize=8, textColor=MUTED, spaceAfter=2*mm)
                        ))
                else:
                    story.append(bullet_item(str(g)))
            story.append(Spacer(1, 3*mm))

        if man_r:
            story.append(sub_heading('Mantra Remedies'))
            for m in man_r:
                if isinstance(m, dict):
                    story.append(gold_heading(m.get('mantra', str(m))))
                    if m.get('benefit'):
                        story.append(prose(m['benefit']))
                    chanting = m.get('chanting', '')
                    if chanting:
                        story.append(Paragraph(
                            f'<i>Practice: {chanting}</i>',
                            ParagraphStyle('ch', parent=styles['small'],
                                           fontSize=8, textColor=MUTED, spaceAfter=2*mm)
                        ))
                else:
                    story.append(bullet_item(str(m)))
            story.append(Spacer(1, 3*mm))

        if life_r:
            story.append(sub_heading('Lifestyle Practices'))
            for l in life_r:
                story.append(bullet_item(str(l)))
            story.append(Spacer(1, 4*mm))
    ...
    if lucky_n or lucky_c:
        ...
        story.append(kv_table(lucky_data))
        story.append(Spacer(1, 4*mm))
    ...
    if num:
        ...
        if num_data:
            story.append(kv_table(num_data))
        if num.get('overview'):
            story.append(prose(num['overview']))
        story.append(Spacer(1, 4*mm))
    ...
    story.append(gold_rule())
    story.append(Paragraph(
        'This report is for reflective and informational purposes only. '
        'Everyday Horoscope · everydayhoroscope.in · SkyHound Studios',
        styles['small']
    ))
