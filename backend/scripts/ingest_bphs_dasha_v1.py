...
INTRO_SLOKAS = {
    # Pure dialog slokas with no prediction content
    "1", "2",
}
...
def should_skip(label: str, text: str) -> bool:
...
def main():
    ...
    # Summary by sub_type and dasha_lord
    sub_types: dict[str, int] = {}
    lords: dict[str, int] = {}
    for r in rules:
        st = r["condition"]["sub_type"]
        sub_types[st] = sub_types.get(st, 0) + 1
        dl = r["condition"].get("dasha_lord", "unknown")
        lords[dl] = lords.get(dl, 0) + 1
    ...
    if len(lords) > 1:
        print("\n  By dasha_lord:")
        for dl, count in sorted(lords.items(), key=lambda x: -x[1]):
            print(f"    {dl:<20} : {count}")
