#!/usr/bin/env python3
"""patch_300combinations_conflicts.py

Reads the positional conflict dedup report for 300 Combinations and applies
...
NOTE: 300 Combinations rules have no planet×position conditions -- positional
conflicts expected = 0. This script handles TF-IDF matches if any are found
against the ~1,797 rules added to DB since the original pre-ingest dedup.

Usage:
  python3 backend/scripts/patch_300combinations_conflicts.py \\
    --mongo-url "$MONGO_URL" \\
    --dry-run
...
REPORT_PATH = Path(
    ...
    "dedup_300combinations_vs_mongodb_positional.json"
)
...
def load_report() -> dict:
    if not REPORT_PATH.exists():
        ...
        print("Run retroactive_dedup_300combinations.sh first.", file=sys.stderr)
        sys.exit(1)
...
def build_patches(polarity_conflicts, sim_matches, contras) -> dict[str, list[str]]:
    ...
    for m in sim_matches:
        ...
        if rule_id:
            note = f"similarity_match vs {m.get('rule_b_id','?')} score={m.get('similarity_score',0):.3f}"
            patches.setdefault(rule_id, []).append(note)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch 300 Combinations positional conflicts in MongoDB."
    )
    ...
    log_path = LOG_DIR / f"patch_300combinations_{ts}{suffix}.log"
    tee = _Tee(log_path)
    sys.stdout = tee
    print("============================================================")
