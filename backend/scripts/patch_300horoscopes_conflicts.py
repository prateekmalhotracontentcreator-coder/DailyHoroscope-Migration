#!/usr/bin/env python3
"""patch_300horoscopes_conflicts.py

Reads the positional conflict dedup report for 300 Horoscopes Vol 1 and applies
...
Usage:
  python3 backend/scripts/patch_300horoscopes_conflicts.py \\
    --mongo-url "$MONGO_URL" \\
    --dry-run
...
REPORT_PATH = Path(
    ...
    "dedup_300horoscopes_vs_mongodb_positional.json"
)
...
def load_report() -> dict:
    if not REPORT_PATH.exists():
        ...
        print("Run retroactive_dedup_300horoscopes.sh first.", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch 300 Horoscopes Vol 1 positional conflicts in MongoDB."
    )
    ...
    log_path = LOG_DIR / f"patch_300horoscopes_{ts}{suffix}.log"
    tee = _Tee(log_path)
    sys.stdout = tee
    print("============================================================")
    ...
    if not patches:
        ...
        print("300 Horoscopes Vol 1: CLEAN ✅")
        return
