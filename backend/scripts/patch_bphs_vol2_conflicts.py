#!/usr/bin/env python3
"""patch_bphs_vol2_conflicts.py

Reads the positional conflict dedup report for BPHS Vol 2 and applies
...
REPORT_PATH = Path(
    ...
    "dedup_bphs_vol2_vs_mongodb_positional.json"
)
...
def load_report() -> dict:
    if not REPORT_PATH.exists():
        ...
        print("Run retroactive_dedup_bphs_vol2.sh first.", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Patch BPHS Vol 2 positional conflicts.")
    ...
    log_path = LOG_DIR / f"patch_bphs_vol2_{ts}{suffix}.log"
    tee = _Tee(log_path)
    sys.stdout = tee
    print("============================================================")
    ...
    if self_matches:
        print(f"WARNING: {len(self_matches)} self-match artifact(s).")
