#!/usr/bin/env python3
"""patch_phaladeepika_positional_conflicts.py

Reads the positional conflict dedup report for Phaladeepika and applies
targeted MongoDB patches.

Triage logic
-----------
  self_match                   → SKIP (stale export dir artifact -- should be 0)
...
Context
-------
  Phaladeepika (Mantreswara) covers similar classical astrology territory
  as BPHS Vol 1 (Parashara). Genuine polarity conflicts expected where
  both texts encode the same planet×house or planet×sign condition but
  with opposite polarity (e.g., Phaladeepika says Jupiter in H7 = auspicious,
  BPHS says Jupiter in H7 = inauspicious in a specific context).

  These are NOT errors -- they represent different classical authorities with
  legitimately differing interpretations. Setting pending_review=True flags them
  for TT review at the co-founder approval stage.
...
REPORT_PATH = Path(
    ...
    "dedup_phaladeepika_vs_mongodb_positional.json"
)
...
def load_report() -> dict:
    if not REPORT_PATH.exists():
        ...
        print("Run retroactive_dedup_phaladeepika.sh first.", file=sys.stderr)
        sys.exit(1)
...
def print_alt_results_summary(alt_results: list) -> None:
    """Print review-only alternate results grouped by positional key."""
    if not alt_results:
        return
    ...
    print("  → No DB patch needed. These are multi-authority complementary results.")
    print()


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Patch Phaladeepika positional conflicts in MongoDB."
    )
    ...
    log_path = LOG_DIR / f"patch_phaladeepika_{ts}{suffix}.log"
    tee = _Tee(log_path)
    sys.stdout = tee
    print("============================================================")
    ...
    total_pos = len(self_matches) + len(polarity_conflicts) + len(alt_results)
    print(f"Positional conflicts in report    : {total_pos}")
    ...
    if self_matches:
        print(f"WARNING: {len(self_matches)} self-match artifact(s) -- export dir was not cleared.")
        for d in self_matches:
            print(f"  ARTIFACT SKIP: {d['rule_a_id']} == {d['rule_b_id']}")
        print()

    print_alt_results_summary(alt_results)

    patches = build_patches(polarity_conflicts, sim_matches, contras)

    if not patches:
        print("No genuine conflicts -- no MongoDB patches required.")
        print("Phaladeepika: CLEAN ✅")
        return

    if args.dry_run:
        print(f"[DRY RUN] {len(patches)} rule(s) WOULD be patched with pending_review=True:")
    ...
    pending_rev = col.count_documents(
        {"source.batch_id": BATCH_ID, "pending_review": True}
    )
    print(f"  pending_review=True (flag):  {pending_rev}")
