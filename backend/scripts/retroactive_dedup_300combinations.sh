#!/bin/bash
# =============================================================================
# Retroactive Positional Conflict Dedup -- 300 Combinations (300-combinations-v1-20260601)
# =============================================================================
# Run from repo root:
#   export MONGO_URL="mongodb+srv://..."
#   bash backend/scripts/retroactive_dedup_300combinations.sh
#
...
#   Contains 22 *Rules*.json files = 329 rules total (batch: 300-combinations-v1-20260601)
#
# Why retroactive?
#   Pre-ingest dedup ran 2026-06-01 against 8,867 rules (found 0 matches).
#   Since then ~1,797 new rules added (Phaladeepika, BPHS Vol 2, longevity batches).
#   Positional conflict detector also post-dates the original dedup (added 2026-06-02).
#   NOTE: 300 Combinations rules have 0 planet×position conditions -- positional
#   detector expected to find 0 conflicts. TF-IDF pass covers the DB delta.
#
# What this does:
#   1. Clears /tmp/mongo_existing_rules_dedup/ (prevents stale-file false-positives)
#      then exports ALL MongoDB rules EXCEPT the 300 Combinations batch (~10,335 rules)
...
if [ -z "${MONGO_URL:-}" ]; then
  ...
  echo "       bash backend/scripts/retroactive_dedup_300combinations.sh"
  exit 1
fi

SCRIPT="backend/ke_dedup_script.py"
FOLDER_A="/Users/apple/Documents/Knowledge Engine_eBooks/ThreeHundredCombinations_CC_Decode"
MONGO_EXPORT_DIR="/tmp/mongo_existing_rules_dedup"
REPORTS="KE_TEXTBOOK_DECODE/Dedup_Reports"
REPORT_PATH="$REPORTS/dedup_300combinations_vs_mongodb_positional.json"
BATCH_ID="300-combinations-v1-20260601"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_PATH="$REPORTS/300combinations_dedup_${TIMESTAMP}.md"
...
echo "============================================================"
echo "300 Combinations -- Retroactive Positional Conflict Dedup"
...
# ------------------------------------------------------------------
# Step 1 -- Export MongoDB (all rules except the 300 Combinations batch)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Step 2 -- Run dedup: 300 Combinations vs full MongoDB export
# ------------------------------------------------------------------
echo "--- STEP 2/3: Run dedup (300 Combinations vs full MongoDB) ---"
python3 "$SCRIPT" \
  --folder-a "$FOLDER_A" \
...
print(f"Rules in 300 Combinations batch (A): {rules_a}")
...
if genuine == 0 and not self_matches:
    ...
    print("    300 Combinations is clear against the full MongoDB.")
    print("    Retroactive dedup gap formally closed.")
else:
    if genuine > 0:
        print("⚠️   REVIEW REQUIRED")
        if polarity_conf or matches or contras:
            print("    Run patch script:")
            print("    python3 backend/scripts/patch_300combinations_conflicts.py \\")
