#!/bin/bash
# =============================================================================
# Retroactive Positional Conflict Dedup -- BPHS Vol 1 Phase 2
#                                          (bphs-vol1-phase2-v1-20260601)
# =============================================================================
# Run from repo root:
#   export MONGO_URL="mongodb+srv://..."
#   bash backend/scripts/retroactive_dedup_bphs_vol1_phase2.sh
#
# Source folder:
#   /Users/apple/Documents/Knowledge Engine_eBooks/BPHS_CC_Decode/
#   Ch03-Ch11, Ch25-26, Ch28-33 = 696 rules total
#   (batch: bphs-vol1-phase2-v1-20260601)
#
# Why retroactive?
#   The positional conflict detector was added 2026-06-02, after this batch
#   was ingested 2026-06-01. BPHS Vol 1 Phase 2 covers Chhs 3-11 and 25-33 --
#   mostly house/planet strength chapters, sign dispositors, and special lagnas.
#   Overlap with Phase 1 (Chs 12-44, ~1,069 rules) and Phaladeepika expected.
#
# What this does:
#   1. Clears /tmp/mongo_existing_rules_dedup/ then exports ALL MongoDB rules
#      EXCEPT bphs-vol1-phase2-v1-20260601 (~9,968 rules)
...
if [ -z "${MONGO_URL:-}" ]; then
  ...
  echo "       bash backend/scripts/retroactive_dedup_bphs_vol1_phase2.sh"
  exit 1
fi

SCRIPT="backend/ke_dedup_script.py"
FOLDER_A="/Users/apple/Documents/Knowledge Engine_eBooks/BPHS_CC_Decode"
MONGO_EXPORT_DIR="/tmp/mongo_existing_rules_dedup"
REPORTS="KE_TEXTBOOK_DECODE/Dedup_Reports"
REPORT_PATH="$REPORTS/dedup_bphs_vol1_phase2_vs_mongodb_positional.json"
BATCH_ID="bphs-vol1-phase2-v1-20260601"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_PATH="$REPORTS/bphs_vol1_phase2_dedup_${TIMESTAMP}.md"
...
echo "============================================================"
echo "BPHS Vol 1 Phase 2 -- Retroactive Positional Conflict Dedup"
...
# ------------------------------------------------------------------
# Step 1 -- Export MongoDB (all rules except BPHS Vol 1 Phase 2)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
# Step 2 -- Run dedup: BPHS Vol 1 Ph2 vs full MongoDB export
# ------------------------------------------------------------------
echo "--- STEP 2/3: Run dedup (BPHS Vol 1 Ph2 vs full MongoDB) ---"
python3 "$SCRIPT" \
  --folder-a "$FOLDER_A" \
...
print(f"Rules in BPHS Vol 1 Phase 2 (A):   {rules_a}")
...
if genuine == 0 and not self_matches:
    ...
    print("    BPHS Vol 1 Phase 2 is clear against the full MongoDB.")
else:
    if polarity_conf:
        print(f"⚠️   {len(polarity_conf)} GENUINE POLARITY CONFLICT(S) -- patch required.")
        print("    python3 backend/scripts/patch_bphs_vol1_phase2_conflicts.py \\")
