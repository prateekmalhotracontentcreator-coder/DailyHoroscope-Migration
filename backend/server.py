...
# Email notification helper (uses Gmail SMTP if configured)
async def send_email_notification(to_email: str, subject: str, body: str):
    """Send email notification — logs if SMTP not configured"""
    import smtplib
    ...
    if not smtp_host or not smtp_user:
        logging.info(f"[EMAIL NOT SENT - SMTP not configured] To: {to_email} | Subject: {subject}")
        logging.info(f"Email body: {body[:200]}")
        return False
    
    try:
        msg = MIMEMultipart('alternative')
        msg['Subject'] = subject
        msg['From'] = f"Everyday Horoscope <{from_email}>"
        msg['To'] = to_email
