...
async def generate_brihat_kundli(request: BrihatKundliRequest, user_email: str = ""):
    ...
    try:
        ...
        # Map Claude JSON output to report model
        # Handle both old structure (flat keys) and new structure (nested remedies/yogas)
        ...
        report = BrihatKundliReport(
            ...
            # Dasha — handle both flat and nested
            current_dasha=dasha if isinstance(dasha, dict) and "current_dasha" not in report_data
                          else report_data.get("current_dasha", dasha if isinstance(dasha, dict) else {}),
            dasha_timeline=report_data.get("dasha_timeline",
                           dasha.get("upcoming", []) if isinstance(dasha, dict) else []),
            # Mangal Dosha — handle nested
            ...
            # Yogas — handle list of dicts with "type" field
            ...
            # Remedies — handle nested or flat
        ...
        # Store in database — use json mode to ensure all types serialisable
        doc = report.model_dump(mode='json')

        # Sanitise any non-serialisable values recursively
        import json
        def sanitise(obj):
            if isinstance(obj, dict):
                ...
                return str(obj)  # convert datetime, etc to string

        doc = sanitise(doc)
        await db.brihat_kundli_reports.insert_one(doc)

        # Return only serialisable fields
