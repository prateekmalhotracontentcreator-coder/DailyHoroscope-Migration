...
Return this EXACT JSON structure with EXACT field names. All string values must be full prose sentences:
{
    ...
    user_prompt = f"""Generate a comprehensive premium Brihat Kundli Pro report for {request.full_name} — equivalent to a 40-page professional astrological report.

Use ONLY this calculated chart data:

{chart_summary}

CRITICAL INSTRUCTIONS:
1. Every section must contain full prose paragraphs — not one-liners. Write like an experienced Jyotishi consulting a client.
2. House analysis is mandatory for ALL 12 houses — each interpretation must be 4-5 sentences.
3. Career timeline must have 3 periods covering 2026 to 2047.
4. Dasha analysis must cover current + next 2 Mahadashas with 5-sentence overviews.
5. Address {request.full_name.split()[0]} by name throughout — personalise every section to the actual chart.
6. Gemstone and mantra remedies must include how-to instructions, not just names.
7. Every array must have minimum 5-8 items with full explanations.
8. Return ONLY valid JSON — no markdown, no preamble."""
    ...
    try:
        ...
        try:
            ...
            logging.error(f"Brihat JSON parse failed: {je}. Raw (first 500): {clean[:500]}")
            # Return minimal valid structure so report still saves
            return {
...
async def generate_brihat_kundli(request: BrihatKundliRequest, user_email: str = ""):
    ...
    try:
        # ── Layer 1: Calculate chart for authoritative data ────────────────────
        chart_data = None
        try:
            chart_data = calculate_vedic_chart(
                date_of_birth=request.date_of_birth,
        ...
        # Generate SVG chart for on-screen display
        chart_svg = ""
        ...
        # Sun sign — add from planets if missing
        ...
        # Dasha — normalise to frontend expected structure
        # Frontend expects: current_dasha.mahadasha, current_dasha.end_year, current_dasha.effects
        current_dasha_raw = report_data.get("current_dasha", dasha)
        if isinstance(current_dasha_raw, str):
            # Strip trailing "Dasha" if already in string to avoid "Jupiter Mahadasha Dasha"
            ...
            # Clean up planet name to avoid double "Dasha"
            if cd.get("mahadasha"):
        ...
        # Dasha timeline — frontend expects [{planet, period, effects}]
        ...
        # If Claude didn't provide dasha dates, fill from calculator
        if chart_data and chart_data.get("dashas"):
            ...
            # Build timeline from next dashas
            if curr_idx >= 0 and len(dasha_timeline) == 0:
                ...
                # Fill missing periods from calculator
        ...
        # Mangal Dosha — calculator data is authoritative, Claude adds interpretation
        ...
        if chart_data and chart_data.get("mangal_dosha"):
            ...
            # Use calculator for factual fields, Claude for descriptive fields
            mangal = {
                ...
                # Add Claude's remedies if available
        ...
        # Serialise to plain JSON-safe dict before storing
        import json
        doc = json.loads(report.model_dump_json())

        # Insert a copy — insert_one adds _id which is not JSON serialisable
        doc_for_db = {**doc}
...
async def download_brihat_kundli_pdf(report_id: str, user_email: str = None):
    ...
    try:
        # Recalculate chart data for the diamond chart (deterministic)
        chart_data = None
        try:
            chart_data = calculate_vedic_chart(
        ...
        # Generate password: FirstName + BirthYear + Month
...
async def generate_kundali_milan(request: KundaliMilanRequest):
    ...
    # Generate SVG charts for both people for on-screen display
    ...
    # Extract individual koota scores from ashtakoot_data if available
...
async def download_kundali_milan_pdf(report_id: str, user_email: str = None):
    ...
    try:
        # Password based on person1 (groom/first person)
...
async def download_birth_chart_pdf(profile_id: str, user_email: str = None):
    ...
    try:
        # Recalculate chart data for PDF (deterministic — always same result)
        chart_data = None
        try:
            chart_data = calculate_vedic_chart(
                date_of_birth=profile['date_of_birth'],
        ...
        # Generate password + PDF with chart data
...
# ── My Reports — unified report list for a user ──────────────────────────────
...
async def get_my_reports(user_email: str, request: Request):
    ...
    await get_current_user(request, db)  # validates session — raises 401 if not logged in
    try:
        reports = []

        # 1. Birth Chart reports — via birth_profiles owned by this user
        ...
        # 2. Kundali Milan reports — person1_id or person2_id in user's profiles
        if profile_ids:
...
async def oauth_callback(response: Response, body: OAuthCallbackRequest):
    """Handle Google OAuth callback — receives Google auth code in body as session_id"""
    code = body.session_id
    if not code:
