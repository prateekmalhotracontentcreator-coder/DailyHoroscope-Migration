...
SPREAD_OPENERS = [
    "This guide approaches the spread as a deliberate reading container rather than a quick novelty draw.",
    "Use this page as a working manual for the spread, especially when the structure matters as much as the cards.",
    "The spread becomes clearest when you read it as a sequence of linked prompts instead of isolated answers.",
    "Think of this spread as a shaped conversation where each position sharpens the next interpretive move.",
    "This layout works best when the full pattern is allowed to emerge before any one card is treated as final.",
    "The spread is designed to hold a specific kind of question, giving the reading more discipline than a casual pull.",
    "Treat this spread as a practical framework for interpretation, not only a symbolic arrangement on the table.",
    "Read the spread slowly enough that its internal logic reveals why each card sits where it does.",
]
SPREAD_POSITION_BRIDGES = [
    "Each position names a separate angle of the situation, so the order of the cards changes the meaning.",
    "Position by position, the layout turns a broad question into smaller symbolic decisions.",
    "The positions work like checkpoints, asking the reader to separate cause, pressure, advice, and outcome.",
    "Every slot in the pattern has a job, which is why the reading gains clarity when the roles stay distinct.",
    "Read the positions as stages in a conversation, with each card answering a different layer of the problem.",
    "The card placements divide the reading into functions, preventing one dramatic symbol from swallowing the whole message.",
    "The position labels create boundaries inside the reading, helping each card speak to its own task first.",
    "The layout gains power when the positions are honoured as separate lenses rather than blended too early.",
]
INTENTION_META_TITLE = {
    "love": "Love Tarot Reading - Spreads, Cards & Relationship Insight",
    "career": "Career Tarot Reading - Promotion, Timing & Work Direction",
    "money": "Money Tarot Reading - Finances, Choices & Resource Insight",
    "health": "Health Tarot Reading - Recovery, Stress & Support Insight",
    "relationships": "Relationship Tarot Reading - Dynamics, Conflict & Clarity",
    "breakup": "Breakup Tarot Reading - Spreads, Healing & Closure Guidance",
    "new-beginnings": "New Beginning Tarot Reading - Spreads, Timing & Momentum",
    "anxiety": "Anxiety Tarot Reading - Grounding, Nervous System & Reflection",
    "decision-making": "Decision Tarot Reading - Options, Consequences & Clear Choice",
    "spiritual-growth": "Spiritual Growth Tarot Reading - Symbols, Practice & Insight",
    "family": "Family Tarot Reading - Patterns, Loyalty & Honest Insight",
    "travel": "Travel Tarot Reading - Journey Timing, Safety & Direction",
    "manifestation": "Manifestation Tarot Reading - Alignment, Desire & Next Steps",
    "self-discovery": "Self Discovery Tarot Reading - Identity, Patterns & Inner Truth",
    "forgiveness": "Forgiveness Tarot Reading - Release, Boundaries & Softening",
    "loss-grief": "Grief Tarot Reading - Loss, Healing & Emotional Support",
    "friendship": "Friendship Tarot Reading - Trust, Reciprocity & Social Clarity",
    "pregnancy": "Pregnancy Tarot Reading - Care, Hope & Reflective Guidance",
    "legal-matters": "Legal Tarot Reading - Fairness, Strategy & Decision Guidance",
    "past-lives": "Past Life Tarot Reading - Karmic Symbols & Memory Insight",
}
...
def _hash_index(*parts: str, modulus: int) -> int:
    import hashlib

    joined = "|".join(parts)
    return int(hashlib.md5(joined.encode()).hexdigest(), 16) % modulus
...
def _use_answer(spread: dict, seed: int) -> str:
    ...
    if "twenty-two" in ulow or "major cards" in ulow or ("major" in ulow and "card" in ulow):
        variants = [
            "Yes. Pull the 22 major arcana cards from the deck and set the minor arcana aside before you shuffle. This spread works within that focused range.",
            "Use the major arcana only - all 22 cards. Separate them from the full deck before you begin so the reading stays within the archetypal layer.",
            "Draw from the 22 major arcana. Their symbolic range fits the scope of this spread better than the full 78-card deck.",
            "Major arcana only (22 cards). Setting the minor arcana aside keeps the reading focused on the broader life-theme language those cards carry.",
        ]
        return variants[seed % 4]
    if "minor" in ulow and "court" in ulow:
        variants = [
            "Use the 56 minor arcana cards - the 40 pip cards and 16 court cards. Leave the major arcana out of the shuffle.",
            "Draw from the minor arcana only (pip and court cards across all four suits). The major arcana are not needed for this layout.",
            "The spread works with the 56-card minor arcana set. Separating them from the major arcana keeps the reading in the everyday-action register.",
            "Pull the minor arcana - the numbered pip cards and all 16 court cards - and set the 22 major arcana aside before shuffling.",
        ]
        return variants[seed % 4]
    full_variants = [
...
def get_spread(slug: str) -> dict[str, Any] | None:
    ...
    opener_variant = int(slug[-1], 36) % len(SPREAD_OPENERS)
    bridge_variant = (len(slug) + card_count) % len(SPREAD_POSITION_BRIDGES)
    when_to_use = [spread["purpose"], SPREAD_OPENERS[opener_variant], SPREAD_POSITION_BRIDGES[bridge_variant]]
    how_to = ["Take a moment to settle your question into one sentence before you shuffle.", f"Lay out {card_count} cards in order, keeping the positions distinct rather than rushing to the conclusion.", SPREAD_POSITION_BRIDGES[(bridge_variant + 3) % len(SPREAD_POSITION_BRIDGES)], "End by summarising the message in plain language and noting one practical next step."]
    use_seed = _hash_index(spread["slug"], "use_ans", modulus=4)
    faq = [{"question": f"What is the {spread['title']} tarot spread for?", "answer": spread["purpose"]}, {"question": f"How many cards are in the {spread['title']} spread?", "answer": q2_answers[q2_seed]}, {"question": f"When should I use the {spread['title']} spread?", "answer": q3_answers[q3_seed]}, {"question": f"Do I need a full deck for the {spread['title']} spread?", "answer": _use_answer(spread, use_seed)}]
    return {"slug": spread["slug"], "title": spread["title"], "chapter": spread["chapter"], "purpose": spread["purpose"], "card_count": card_count, "positions": positions, "diagram": positions, "how_to": how_to, "sample_reading": sample_reading, "when_to_use": when_to_use, "faq": faq, "meta_title": f"{spread['title']} Tarot Spread -- {spread['chapter']} Layout Guide", "meta_description": f"Learn the {spread['title']} tarot spread: layout, positions, when to use it, and a sample reading walkthrough.", "schema_howto_steps": [{"name": "Clarify the question", "text": how_to[0]}, {"name": "Lay the cards", "text": how_to[1]}, {"name": "Read each position", "text": how_to[2]}, {"name": "Summarise the guidance", "text": how_to[3]}]}
...
def get_card(slug: str) -> dict[str, Any] | None:
    ...
    arcana_label = card["arcana"] if card["arcana"] == "major" else card["suit"].title()
    return {**card, "arcana": arcana_label, "best_spreads": [{"slug": spread["slug"], "title": spread["title"]} for spread in recommended], "faq": faq, "meta_title": meta_title, "meta_description": f"Learn {card['name']} tarot meanings upright and reversed, plus love, career, health, and symbolic interpretations."}
...
def get_intention(slug: str) -> dict[str, Any] | None:
    ...
    faq = [{"question": f"Which tarot spread is best for {payload['label'].lower()}?", "answer": int_q1_answers[int_q1_seed]}, {"question": f"Can tarot help with {payload['label'].lower()}?", "answer": INTRO_SNIPPETS.get(slug, "Tarot helps by revealing patterns, motives, and the next honest step.")}, {"question": f"What cards are favourable for {payload['label'].lower()} readings?", "answer": (", ".join(card['name'] for card in best_cards[:5]) + ".") if best_cards else "Supportive cards usually show openness, clarity, and movement."}, {"question": f"What cards signal caution in {payload['label'].lower()} spreads?", "answer": (", ".join(card['name'] for card in caution_cards[:4]) + ".") if caution_cards else "Caution cards usually highlight fear, delay, conflict, or self-deception."}, {"question": f"Should I still use the interactive tarot tool for {payload['label'].lower()}?", "answer": int_q5_answers[int_q5_seed]}]
    return {"slug": slug, "label": payload["label"], "intro": INTRO_SNIPPETS.get(slug, "Tarot helps by revealing patterns, motives, and the next honest step."), "top_spreads": [spread for spread in spreads if spread], "best_cards": [{"slug": card['slug'], "name": card['name'], "meaning": card['upright']} for card in best_cards], "caution_cards": [{"slug": card['slug'], "name": card['name'], "meaning": card['reversed']} for card in caution_cards], "sample_walkthrough": walkthrough, "faq": faq, "meta_title": INTENTION_META_TITLE.get(slug, f"{payload['label']} Tarot Reading - Spreads and Guidance"), "meta_description": f"Explore the best tarot spreads for {payload['label'].lower()}, plus supportive cards, caution cards, and a sample reading walkthrough."}
