...
def calculate_ashtakoot(nak1: str, sign1: str, nak2: str, sign2: str) -> dict:
    ...
    # 3-11 and 5-9 are GOOD despite counting to same numbers
    sign_idx1 = SIGN_ORDER.index(sign1)
    sign_idx2 = SIGN_ORDER.index(sign2)
    diff = abs(sign_idx1 - sign_idx2) + 1
    # Normalise to smaller of the two directions (1-6 scale)
    diff_norm = diff if diff <= 6 else 13 - diff
    # Only 6-8 axis (diff_norm=6) and 2-12 axis (diff_norm=2) are inauspicious
    bhakoot_bad = diff_norm in [2, 6]
