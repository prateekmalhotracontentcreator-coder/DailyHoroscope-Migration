  ...
  const checkSubscription = async () => {
    try {
      const response = await axios.get(`${API}/premium/check`, {
        params: { user_email: user.email, report_type: 'premium_monthly', report_id: 'subscription' }
      });
      if (response.data.has_premium_access) {
        setSubscription({ active: true, type: 'Premium Monthly', amount: '\u20b91,599/month' });
      }
    } catch (error) {
      ...
      <DropdownMenuContent className="w-80" align="end" forceMount>
        ...
        <div className="px-2 py-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {isPremium ? (
              <span className="text-xs bg-gold text-primary-foreground px-2 py-1 rounded-full font-semibold">{subscription.type}</span>
            ) : (
              <span className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-full font-semibold">Free</span>
            )}
          </div>

          {isPremium ? (
            <div className="space-y-2 text-xs">
              {['Unlimited Birth Charts', 'Unlimited Kundali Milan', 'PDF Downloads', 'Social Sharing'].map(f => (
                <div key={f} className="flex items-center text-green-600 dark:text-green-400">
                  <Check className="h-4 w-4 mr-2" /><span>{f}</span>
                </div>
              ))}
              <div className="pt-2 border-t border-border">
                <p className="text-muted-foreground">Billed {subscription.amount}</p>
              </div>
            </div>
          ) : (
            <div className="space-y-2 text-xs">
              {['Daily/Weekly/Monthly Horoscopes', 'Basic Birth Chart Preview'].map(f => (
                <div key={f} className="flex items-center text-green-600 dark:text-green-400">
                  <Check className="h-4 w-4 mr-2" /><span>{f}</span>
                </div>
              ))}
              {['Full Birth Chart Analysis', 'Kundali Milan Analysis', 'PDF Downloads'].map(f => (
                <div key={f} className="flex items-center text-muted-foreground">
                  <X className="h-4 w-4 mr-2" /><span>{f}</span>
                </div>
              ))}
            </div>
          )}
