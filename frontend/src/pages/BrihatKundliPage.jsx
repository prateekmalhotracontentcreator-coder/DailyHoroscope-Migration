...
export const BrihatKundliPage = () => {
  ...
  // Check premium access on mount — same pattern as BirthChartDisplay
  useEffect(() => {
    const checkPremium = async () => {
      if (!user?.email) { setPremiumChecking(false); return; }
      try {
        const res = await axios.get(
          `${API}/premium/check?user_email=${encodeURIComponent(user.email)}&report_type=brihat_kundli&report_id=new`,
          { withCredentials: true }
        );
        setIsPremium(res.data.has_access || false);
      } catch {
        setIsPremium(false);
      } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <SEO
        title="Brihat Kundli Pro — Comprehensive Vedic Life Report"
        description="Get your 40+ page Brihat Kundli Pro report. Full planetary positions, dasha timeline, career and love predictions, dosha analysis, gemstone remedies, and numerology — all in one Vedic astrology report."
