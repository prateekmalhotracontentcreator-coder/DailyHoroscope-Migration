...
export const Home = () => {
  ...
  useEffect(() => {
    const sign = localStorage.getItem('selected-sign');
    if (sign && ZODIAC_META[sign]) setSavedSign(sign);
  }, []);

  const signMeta = savedSign ? ZODIAC_META[savedSign] : null;

  const features = [
    {
      id: 'daily',
      title: 'Daily Horoscope',
      description: 'Get your personalised daily cosmic guidance',
      icon: Sun,
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        {signMeta ? (
          // User has a saved sign — show personalised banner
          <div className="mb-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-xl border border-gold/30 bg-gradient-to-r from-gold/5 via-card to-gold/5">
              <div className="flex items-center gap-4">
                <div className="text-5xl leading-none">{signMeta.symbol}</div>
                <div>
                  ...
                  <h2 className="text-2xl font-playfair font-semibold">
                    Your sign: {signMeta.name}
                  </h2>
                  <p className="text-sm text-muted-foreground">{signMeta.element} sign · Your cosmic guidance awaits</p>
                </div>
              </div>
