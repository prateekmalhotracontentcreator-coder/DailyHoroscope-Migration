...
import {
  ...
  Crown, ChevronDown, ArrowRight, Check, Moon
} from 'lucide-react';
...
const PLANS = [
  ...
  },
  {
    ...
    features: ['Everything in Free', 'Unlimited Birth Chart reports', 'Unlimited Kundali Milan', 'PDF downloads', 'Priority AI generation', 'Social sharing'],
  },
  {
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/40 bg-background/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-sm text-muted-foreground">
            {[['Features', 'features'], ['Pricing', 'pricing'], ['Blog', null]].map(([label, id]) => (
              <button key={label} onClick={() => id ? document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' }) : navigate('/blog')} className="hover:text-foreground transition-colors">{label}</button>
            ))}
          </div>
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center pt-16 pb-12 px-4 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto">
          <div className="inline-flex items-center gap-2 border border-gold/30 bg-gold/5 text-gold text-xs font-semibold uppercase tracking-[0.2em] px-4 py-2 rounded-full mb-8">
            <Moon className="h-3 w-3" /> AI-Powered Vedic Astrology
          </div>
          ...
          <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 leading-relaxed font-playfair">
            Personalised daily horoscopes, birth charts, and Vedic reports — powered by AI and the ancient wisdom of the cosmos.
          </p>
